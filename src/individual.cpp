...
double ParentStutterContribution(const std::size_t & currentAllele, const std::size_t & stutterRecursion, const Eigen::VectorXd & decodedProfile_mu, const std::vector<Eigen::MatrixXd> & potentialParents_m, const double & numberOfAlleles_m)
{
    ...
    if (potentialParentIndex.sum() != -1)
    {
        ...
        {
            double parentContribution = ParentStutterContribution(i, stutterRecursion - 1, decodedProfile_mu, potentialParents_m, numberOfAlleles_m);
            potentialParentContribution[i] = stutterContribution[i] * decodedProfile_mu[potentialParentIndex[i] - 1] + parentContribution;
        }
    }
...
Eigen::MatrixXd Individual::GenerateExpectedContributionProfile(const ExperimentalSetup & ES, const Eigen::MatrixXd & decodedProfile)
{
    ...
    for (std::size_t m = 0; m < ES.NumberOfMarkers; m++)
    {
        ...
        for (std::size_t u = 0; u < ES.NumberOfContributors; u++)
        {
            ...
            {
                stutterContribution[a] = ParentStutterContribution(a, ES.LevelsOfStutterRecursion, decodedProfile_mu, potentialParents_m, ES.NumberOfAlleles[m]);
            }
...
Rcpp::List setupIndividual(const std::size_t & numberOfMarkers, const Eigen::VectorXd & numberOfAlleles, const std::size_t & numberOfContributors,
    ...
    const ExperimentalSetup ES(numberOfMarkers, numberOfAlleles, numberOfContributors, numberOfKnownContributors, knownProfiles, allProfilesEmpty, coverage, potentialParents, markerImbalances, tolerance, theta, alleleFrequencies, levelsOfStutterRecursion);
    Individual I(ES);
